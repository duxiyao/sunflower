...
import com.dxy.samples.apps.sunflower.data.Response
    ...
    override fun startWork(intent: Intent?, flags: Int, startId: Int) {
        ...
        sDisposable = Observable
                ...
                .subscribe { count: Long ->
                    println("Observable---"+Thread.currentThread().priority)
...
//                    getAction()
    ...
    fun getAction(){
        println("getAction---"+Thread.currentThread().priority)
        GlobalScope.launch {
            try {
                println("GlobalScope---"+Thread.currentThread().priority)
